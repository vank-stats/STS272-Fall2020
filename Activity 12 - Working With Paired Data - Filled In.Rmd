---
title: "Activity 12 - Working With Paired Data"
...
The example we'll look at is one from an experiment conducted by Charles Darwin. He wanted to look at the heights of cross-fertilized versus self-fertilized plants (measured in inches to the nearest 1/8th of an inch). To do this, he took 15 pots and potted one plant of each type in each pot. The plants were then measured after a certain period of time, and their heights were recorded. Below is code to read the data into R.
...
1. What kinds of numerical and/or graphical summaries might we be interested in for this data? Include code for both below.
...
2. What do you notice? Did you make use of the `pot` variable at all?

It looks like the cross fertilized plans tend to be taller (this is seen in Q1, Median, Mean, Q3, and max in the numeric summary and in our graphs).
...
```{r}
get_ci(darwin_boot, level = 0.9, type = "percentile") %>%
  data.frame()
```

My 90% CI is: (0.57, 4.45)

(Note: The interval above might be slightly different for you. They differed between my home and work computers.)
...
- Entirely positive CI: We think cross-fertilized plants are taller on average (higher pop. mean)
- Entirely negative CI: We think cross-fertilized plants are shorter on average (or in other words self-fertilized are taller)
- Interval contains + and - values: We aren't really confident which group is taller on average
...
We are 90% confident that population mean height of cross-fertilized plants is higher than the population mean for self-fertilized plants by between 0.57 and 4.45 inches.
...
There is a dataset on Moodle called **twins.csv**. The data is from a 1990 study where researchers compared left hippocampus volumes (in cm^3) for twins where one twin had schizophrenia (column `affected`) and the other twin did not (column `unaffected`). 
